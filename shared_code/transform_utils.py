...
# Drop complete OAT events by (eventId, eventSubId) combination.
# NOTE: eventId/eventSubId are expected under log["detail"].
DROPPED_EVENT_COMBINATIONS = {
    3: {201, 202, 203, 204},  # TELEMETRY_CONNECTION
    4: {301},  # TELEMETRY_DNS
    7: {601, 602, 603},  # TELEMETRY_INTERNET
    10: {0},  # TELEMETRY_WINDOWS_EVENT
}
...
def transform_oat_log(clp_id, log):
    # Filter out unwanted events early (before transform / oversize processing).
    detail = (log or {}).get("detail", {})
    event_id = detail.get("eventId")
    event_sub_id = detail.get("eventSubId")

    if (
        event_id in DROPPED_EVENT_COMBINATIONS
        and event_sub_id in DROPPED_EVENT_COMBINATIONS[event_id]
    ):
        return None
